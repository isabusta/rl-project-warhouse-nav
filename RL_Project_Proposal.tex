...
\section{Domain}
We consider a warehouse where a robot must pick up packages and deliver them to different locations while navigating around obstacles. The warehouse is represented as a grid and the problem is to figure out which package to deliver first and how to navigate efficiently across all deliveries. We model this as a Markov Decision Process (MDP), where the agent learns through trial and error to find the best delivery strategy.
...
In each experiment-setup, the robot receives $N$ packages that must be delivered to designated storage positions. The number $N$ can be configures individual by the user. 
...
\begin{itemize}
    \item movement actions (up, down, left, right),
    \item package pickup,
    \item package storage.
\end{itemize}
...
The reward function is designed to encourage efficient task completion:
...
A successful learning method should minimize total completion time, penalty factors and maximize the cumulative reward.
